...
Points at the locally generated USDs (``assets/bdash/``, produced by
...
Geometry lives in ``configs/bdash/assets.yaml`` and is re-read here rather than duplicated, so
...
BDASH_ASSET_DIR = os.path.join(_REPO_ROOT, "assets", "bdash")
BDASH_ASSET_CONFIG = os.path.join(_REPO_ROOT, "configs", "bdash", "assets.yaml")
...
# Same rationale as the vdash peg (docs/physics_params.md): the default high-precision props use
...
class _BDashWorkpiece(LibraryObject):
    ...
    ``variant_id`` keys back into configs/bdash/assets.yaml, so per-variant tolerances and
