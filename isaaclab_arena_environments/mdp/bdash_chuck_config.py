...
Mirrors :mod:`isaaclab_arena_environments.mdp.bdash_peg_config`: scene poses, tray spawn ranges,
...
# repo root = .../IsaacLab-Arena-An  (this file is isaaclab_arena_environments/mdp/bdash_chuck_config.py)
_REPO_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
BDASH_CONFIG_DIR = os.path.join(_REPO_ROOT, "configs", "bdash", "chuck_load")
...
def load_task_cfg(path: str | None = None) -> dict:
    """Read ``configs/bdash/chuck_load/task.yaml`` (or an override path) into a dict. Cached per-path."""
...
def load_controllers_cfg(path: str | None = None) -> dict:
    """Read ``configs/bdash/chuck_load/controllers.yaml`` (pick / touch-off / chuck-load gains).
