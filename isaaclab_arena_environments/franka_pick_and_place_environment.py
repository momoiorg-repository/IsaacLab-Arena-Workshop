...
class FrankaPickAndPlaceEnvironment(ExampleEnvironmentBase):
    ...
    def get_env(self, args_cli: argparse.Namespace):  # -> IsaacLabArenaEnvironment:
        ...
        # NOTE: Must be called before ObjectReference is constructed so that destination_location's
        # event_cfg is built with the correct composed pose (T_W_P × T_P_O). If set_initial_pose is
        # called after ObjectReference.__init__, destination_location.event_cfg captures the
        # uninitialized parent pose (0, 0, 0) and overrides the container's correct reset event.
