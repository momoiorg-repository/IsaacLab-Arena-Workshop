...
"""B-DASH pick→insert environment (brief §3).
...
:class:`~isaaclab_arena.tasks.bdash_pick_insert_task.BDashPickInsertTask` (§3.4 predicates as
terminations + §3.5 per-episode handoff log) and the FORGE-grade physics callback
(:func:`~isaaclab_arena_environments.mdp.env_callbacks.bdash_assembly_env_cfg_callback`).

The clearance ``--clearance`` (mm) selects one of the registered ``bdash_socket_c{...}`` assets and is
...
class BDashPickInsertEnvironment(ExampleEnvironmentBase):

    name: str = "bdash_pick_insert"
    ...
    def get_env(self, args_cli: argparse.Namespace):  # -> IsaacLabArenaEnvironment:
        ...
        from isaaclab_arena.tasks.bdash_pick_insert_task import BDashPickInsertTask
        from isaaclab_arena.utils.pose import Pose
        from isaaclab_arena_environments import mdp
        from isaaclab_arena_environments.mdp import bdash_peg_assets, bdash_peg_randomization
        ...
        socket_name = bdash_peg_assets.BDASH_SOCKET_BY_CLEARANCE[clearance_mm]
        clearance_m = clearance_mm / 1000.0

        # --- assets ------------------------------------------------------------------------------
        background = self.asset_registry.get_asset_by_name(args_cli.background)()
        socket = self.asset_registry.get_asset_by_name(socket_name)()
        peg = self.asset_registry.get_asset_by_name("bdash_peg")()
        ...
        task = BDashPickInsertTask(
            ...
            # BDASH_EPISODE_S overrides the episode budget (default off) — used to give the tactile
            # CALIBRATE probe room for its touch sequence before the insert; headline path unchanged.
            episode_length_s=float(os.environ.get("BDASH_EPISODE_S") or task_cfg["termination"]["episode_length_s"]),
        ...
        return IsaacLabArenaEnvironment(
            ...
            env_cfg_callback=mdp.bdash_assembly_env_cfg_callback,
        )
