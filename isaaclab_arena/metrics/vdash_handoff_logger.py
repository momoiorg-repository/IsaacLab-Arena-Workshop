...
"""V-DASH §3.5 per-episode handoff / terminal-error logger.
...
The thresholds/geometry come from ``configs/vdash/task.yaml`` (passed in via the cfg) so nothing here
...
class VDashHandoffRecorder(RecorderTerm):
    """Accumulates §3.5 per-episode statistics and writes one JSONL record per episode."""

    def __init__(self, cfg: VDashHandoffRecorderCfg, env):
        ...
        self._path = cfg.output_path or os.path.join(
            cfg.output_dir, f"vdash_handoff_{cfg.run_tag}_{ts}.jsonl"
        )
        self._fh = None
    ...
    def record_post_step(self):
        ...
        self.force_insert_max = torch.where(in_insert_window, torch.maximum(self.force_insert_max, f), self.force_insert_max)
        ...
        if os.environ.get("VDASH_DEBUG") and int(step[0].item()) < 6:
            tip = vp._peg_tip_w(env, self._names["peg"], self._tip_offset)
            axis_xy, mouth_z = vp._hole_frame(env, self._names["socket"], self._mouth_h)
            ...
            print(
                f"[VDASH_DEBUG] step={int(step[0].item())} tip_z={tip[0,2].item():.4f} "
    ...
    def _open_log(self):
        ...
        fallback = os.path.join(tempfile.gettempdir(), "vdash", os.path.basename(self._path))
        last_err = None
        for path in (self._path, fallback):
            try:
                ...
                if path != self._path:
                    print(
                        f"[VDashHandoffLogger] '{self._path}' not writable ({last_err}); "
                ...
                last_err = e
        print(f"[VDashHandoffLogger] no writable log location ({last_err}); §3.5 logging disabled.", flush=True)
        return None


@configclass
class VDashHandoffRecorderCfg(RecorderTermCfg):
    class_type: type[RecorderTerm] = VDashHandoffRecorder
    name: str = "vdash_handoff"
    task_cfg: dict = None
    clearance: float = 0.002
    output_dir: str = "logs/vdash"
    output_path: str | None = None
    run_tag: str = "run"


class VDashHandoffLogger(MetricBase):
    """Metric wrapper that installs :class:`VDashHandoffRecorder` and reports the logged success rate.
    ...
    name = "vdash_handoff_success_rate"
    recorder_term_name = "vdash_handoff"

    def __init__(self, task_cfg: dict, clearance: float, output_dir: str = "logs/vdash", run_tag: str = "run"):
    ...
    def get_recorder_term_cfg(self) -> RecorderTermCfg:
        return VDashHandoffRecorderCfg(
