...
"""B-DASH scripted expert policy: scripted pick → rule-based insertion (brief §3.3).
...
pick controller drives. This is the rule-based arm of the B-DASH hierarchy used to measure the
clearance / scene-diversity success curves (M5) and, with the switcher, the ablations (M6).

Registered as policy ``bdash_scripted`` (use with ``policy_runner.py --policy_type bdash_scripted``).
"""
...
@dataclass
class BDashScriptedPolicyArgs:
    @classmethod
    def from_cli_args(cls, args: argparse.Namespace) -> BDashScriptedPolicyArgs:
        return cls()


@register_policy
class BDashScriptedPolicy(PolicyBase):

    name = "bdash_scripted"
    config_class = BDashScriptedPolicyArgs

    def __init__(self, config: BDashScriptedPolicyArgs):
    ...
    def get_action(self, env: gym.Env, observation: GymSpacesDict) -> torch.Tensor:
        ...
        if os.environ.get("BDASH_GRASP_DIAG"):  # grasp-quality diag (mirror of bdash_vla_policy)
            import math
            ...
            if not getattr(self, "_gd_handoff_logged", False) and bool(self._in_insertion[0]):
                ...
                if os.environ.get("BDASH_FINGER_DIAG"):
                    try:
    ...
    def reset(self, env_ids: torch.Tensor | None = None) -> None:
        ...
        self._step = 0
        if os.environ.get("BDASH_GRASP_DIAG"):
    ...
    def _dbg(self, env):
        ...
        print(
            f"[BDASH_DEBUG] step={self._step} pick_ph={int(self._pick.phase[0])} "
    ...
    @staticmethod
    def from_args(args: argparse.Namespace) -> BDashScriptedPolicy:
        return BDashScriptedPolicy(BDashScriptedPolicyArgs.from_cli_args(args))
