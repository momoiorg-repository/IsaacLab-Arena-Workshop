...
"""B-DASH VLA + rule-based insertion switcher policy (the brief's M6 hierarchy for VLA eval).
...
spiral search), exactly as ``bdash_scripted`` does — only the pick stage is the VLA instead of
``ScriptedPick``.
...
Run single-env (non-tiled cameras render one env):
    ...
    policy_runner.py --policy_type isaaclab_arena.policy.bdash_vla_policy.BDashVLAPolicy \\
...
@dataclass
class BDashVLAPolicyArgs:
    policy_config_yaml_path: str = ""
    policy_device: str = "cuda"
    num_envs: int = 1

    @classmethod
    def from_cli_args(cls, args: argparse.Namespace) -> BDashVLAPolicyArgs:
        return cls(
...
class BDashVLAPolicy(PolicyBase):
    ...
    def __init__(self, config: BDashVLAPolicyArgs):
        ...
        self._gate = None  # precision-budget handoff gate (BDASH_BUDGET_GATE); set in _ensure

    # ----------------------------------------------------------------- setup
    def _ensure(self, env):
        if self._ins is not None:
            return
        import isaaclab_arena_environments.mdp.bdash_peg_predicates as _vp  # noqa: F401  (predicate module)
        from isaaclab_arena.controllers.insertion_controller import InsertionController
        from isaaclab_arena_environments.mdp.bdash_peg_config import load_controllers_cfg, load_task_cfg
        ...
        if os.environ.get("BDASH_VLA_SCRIPTED_PICK"):
    ...
    def get_action(self, env: gym.Env, observation) -> torch.Tensor:
        ...
        if os.environ.get("BDASH_VLA_DEBUG"):  # TEMP: where is the gripper vs the peg, and does it close?
        ...
        if os.environ.get("BDASH_GRASP_DIAG"):  # grasp-quality diag: how does the peg sit IN the gripper?
        ...
        if os.environ.get("BDASH_VLA_FORCE_INSERT"):  # debug: exercise the IK->joint bridge from step 0
            self._in_insertion[:] = True
        ins_ik = self._ins.step(uenv, active=self._in_insertion)  # (N, 7) relative-IK
        if os.environ.get("BDASH_BACKSWITCH") and getattr(self._ins, "gave_up", None) is not None:
    ...
    def reset(self, env_ids: torch.Tensor | None = None) -> None:
        ...
        if os.environ.get("BDASH_VLA_DEBUG") or os.environ.get("BDASH_GRASP_DIAG"):  # per-episode segmentation
    ...
    def add_args_to_parser(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        group = parser.add_argument_group("BDash VLA Policy", "GR00T VLA + rule-based insertion switcher")
        group.add_argument(
    ...
    @staticmethod
    def from_args(args: argparse.Namespace) -> BDashVLAPolicy:
        return BDashVLAPolicy(BDashVLAPolicyArgs.from_cli_args(args))
