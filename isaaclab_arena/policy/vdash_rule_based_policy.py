...
"""V-DASH "rule-based 単体" baseline policy (brief §3 M8).

Same scripted pick → rule-based insertion as ``vdash_scripted``, except the **grasp point comes from
classic vision** (top-down colour segmentation + known-plane back-projection,
:class:`~isaaclab_arena.controllers.vdash_classic_vision.ClassicVisionGrasp`) instead of the
...
Registered as ``vdash_rule_based``. Run single-env (the non-tiled overhead camera renders one env)."""
...
from isaaclab_arena.controllers.vdash_classic_vision import ClassicVisionGrasp
from isaaclab_arena.policy.policy_base import PolicyBase


@dataclass
class VDashRuleBasedPolicyArgs:
    @classmethod
    def from_cli_args(cls, args: argparse.Namespace) -> "VDashRuleBasedPolicyArgs":
        return cls()


@register_policy
class VDashRuleBasedPolicy(PolicyBase):

    name = "vdash_rule_based"
    config_class = VDashRuleBasedPolicyArgs

    def __init__(self, config: VDashRuleBasedPolicyArgs):
        ...
        self._vision_steps = int(os.environ.get("VDASH_VISION_STEPS", "8"))
        self._settled = False
        self._step = 0
        self._adim = 7
    ...
    def _ensure(self, env):
        ...
        self._ins = InsertionController(
            names, mouth_height, cc["ee_control"], cc["insertion"], grip_offset=grip_offset
        )
    ...
    @staticmethod
    def from_args(args: argparse.Namespace) -> "VDashRuleBasedPolicy":
        return VDashRuleBasedPolicy(VDashRuleBasedPolicyArgs.from_cli_args(args))
