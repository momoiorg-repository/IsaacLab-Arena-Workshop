...
# SPDX-License-Identifier: Apache-2.0
"""B-DASH pick→insert task (brief §3.4 / §3.5).
...
from the §3.4 *predicates* (:mod:`isaaclab_arena_environments.mdp.bdash_peg_predicates`) and records the
...
Key differences from ``AssemblyTask`` (see plan §8 reconciliation):
  ...
  * all thresholds come from ``configs/bdash/peg_insert/task.yaml`` and the active socket's clearance is injected
...
from isaaclab_arena.metrics.bdash_handoff_logger import BDashHandoffLogger
...
class BDashPickInsertTask(TaskBase):
    ...
    def __init__(
        ...
        # The §3.4 scene keys for peg/socket are the *registered asset names* (e.g. ``bdash_peg``,
        # ``bdash_socket_c1``), which vary per build — override the yaml placeholders with the real
    ...
    def get_mimic_env_cfg(self, arm_mode):
        raise NotImplementedError("B-DASH pick-insert does not use Mimic datagen in this scope.")
...
class EventsCfg:
    ...
    supplied by :mod:`isaaclab_arena_environments.mdp.bdash_peg_randomization`; left ``None`` they are
