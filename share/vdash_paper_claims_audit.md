# V-DASH Paper-Claims Verification Audit

**Date:** 2026-06-26 · **Mode:** read-only (no code/behavior changed) · **Branch:** `an/vdash-vla-training`
...
| 1 | Peg ground-truth used only for predicates/logging | `vdash_predicates.py:121-136`, `vdash_handoff_logger.py:150-189` | **VERIFIED** |
| 2 | Handoff switch = §3.4 state predicate ("verifiable") | VLA: `vdash_vla_policy.py:202` / Scripted: `vdash_scripted_policy.py:85` | **PARTIAL** — true for VLA path; **scripted & M8 baselines switch on `pick_finished`, not the predicate** |
| 2 | Predicate signals | `vdash_predicates.py:89-118` | **CAVEAT** — evaluated on **ground-truth** peg pose+velocity; **no "upright"/tilt term** |
...
| 4 | M8 baseline grasp = vision, no GT/yaw leak | `vdash_rule_based_policy.py:91-97`, `vdash_classic_vision.py:8,33` | **VERIFIED** |
...
| 6 | Physics = Factory/FORGE-derived, no drift | `env_callbacks.py:92-115`, `configs/vdash/assets.yaml`, `vdash_assets.py:36` | **VERIFIED w/ 2 documented deltas** (decimation 2≠8; contact_offset 0.0002≠doc 0.005) |
...
**Provenance.** The 49/50 at c2.0/L1 appears in two runs, both `vdash_scripted`:

- **Headline (most recent):** `results/vdash/eval_grid.csv` (written 2026-06-26 09:52):
  `vdash_scripted,2.0,L1,50,49,0.98,0.895,0.9965,0.605,50` → jsonl
  `logs/vdash/vdash_handoff_grid_vdash_scripted_c2.0_L1_20260626_005107.jsonl`
...
- **Earlier:** `results/vdash/e2_clearance.csv` (working tree) L1 → `..._20260621_111959.jsonl`, also 49/50.

- **n = 50** — `configs/vdash/eval_grid.yaml: num_episodes: 50`; jsonl has exactly 50 lines.
- **seed = 0** — `eval_grid.yaml: seed: 0`; `run_eval_grid.py` passes `--seed 0`. **Single seed.**
- **Scripted, not VLA** — policy `vdash_scripted` (`VDashScriptedPolicy`). For non-VLA policies
...
- success termination = `inserted()` (`vdash_predicates.py:121-136`) reads peg tip + socket + clearance.
- `vdash_handoff_logger.py:150-189` reads `peg.root_quat_w`/tip for handoff dx/dy/tilt, depth, final tilt.
- `vdash_scripted_policy.py:104-124` `_dbg()` reads peg pose — **debug print only.**
- None of these feed the action.

### ⚠ Discrepancy 1 — "current controllers.yaml, no uncommitted local changes" is FALSE
- `git diff configs/vdash/controllers.yaml`: **+24/−2 uncommitted** — adds the M5a `use_hop_search:`
...
- **VLA hierarchy (the paper's VLA results):** `vdash_vla_policy.py:202`
...
- **Scripted policy (the 98% headline):** `vdash_scripted_policy.py:84-85`
...
- **M8 rule-based baseline:** `vdash_rule_based_policy.py:103` — also `pick_finished`. **NOT the predicate.**
...
**What the predicate reads (`vdash_predicates.py:89-118`):** grasped ∧ tip in cylinder ∧ speed<max,
...
(handoff `vdash_handoff_logger.py:154-162`; final `:185-189`) and `|dxy|` from tip−socket_axis at the
...
§6 is the *EE→peg tracking distance* from the `VDASH_VLA_DEBUG` stream, a different quantity than the
...
- **M8 rule-based baseline** — `vdash_rule_based_policy.py:91-97`: grasp point comes from
  `ClassicVisionGrasp` (colour segmentation + plane back-projection), cached to `grasp_override`. The
  vision module docstring (`vdash_classic_vision.py:8,33`) states it must not use ground-truth peg pose;
...
- Recorded dataset `datasets/vdash/vla_pick_handoff_v5_recovery.hdf5` (the v6 training set):
...
- Record-path runtime: `logs/vdash/smoke_fix.out` `framediff` 0.68–3.2 (post-fix) vs
  `smoke_v5_diag.out` `live-framediff=0.0000` (the pre-fix frozen state). ✅ clean contrast.
- Eval-path runtime: `logs/vdash/eval_v6_recovery.out` shows the VLA driving EE→peg `dxy` 0.081→0.010
...
**SimulationCfg** (`env_callbacks.py:92-115`, `vdash_assembly_env_cfg_callback`, wired at
`vdash_pick_insert_environment.py:125`) matches `docs/physics_params.md` §1:
...
**Per-asset:** peg `max_depenetration_velocity = 1.0` (`vdash_assets.py:36`) matches §7 M3b. Socket SDF
`sdf_resolution: 1024`, `rest_offset: 0.0` (`configs/vdash/assets.yaml:27,29`). ✅
...
2. **contact_offset = 0.0002 m** (`configs/vdash/assets.yaml:26`), vs `physics_params.md` §2 which lists
...
- `configs/vdash/assets.yaml` (the source-of-truth socket params) is **unmodified**.
- `assets/vdash/config.yaml` *is* modified, but it is the **MeshConverter per-asset record** for the
...
(`vdash_randomization.py:9-13`) — none of which the controller perceives. The CSV shows L2≡L3 (both
...
`results/vdash/e4_convergence_c3.0.csv` is **byte-identical** to `e4_convergence_c2.0.csv` and its
`clearance_mm` column reads **2.0**. `vdash_pick_insert_environment.py:50,163-165` snaps `--clearance`
...
`.../vdash_classic_vision.py`; `isaaclab_arena/policy/{vdash_scripted_policy,vdash_vla_policy,vdash_rule_based_policy}.py`;
`isaaclab_arena/metrics/vdash_handoff_logger.py`; `isaaclab_arena/evaluation/policy_runner.py`;
`isaaclab_arena_environments/mdp/{vdash_predicates,env_callbacks,vdash_assets,vdash_randomization}.py`;
`isaaclab_arena_environments/vdash_pick_insert_environment.py`;
`isaaclab_arena_gr00t/embodiments/franka/{franka_modality_config.py,gr00t_9dof_joint_space.yaml}`;
`configs/vdash/{controllers.yaml,eval_grid.yaml,assets.yaml}`; `scripts/vdash/{run_eval_grid,run_convergence_zone,record_vla_demos}.py`;
`results/vdash/{eval_grid,e2_clearance,e4_convergence_c2.0,e4_convergence_c3.0,e4_recover_c2.0}.csv`;
`logs/vdash/*.jsonl`, `*.out`; `datasets/vdash/vla_pick_handoff_v5_recovery.hdf5`; `docs/physics_params.md`, `docs/milestones/M10.md`.
