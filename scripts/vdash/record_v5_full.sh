...
#     'cd /workspaces/isaaclab_arena && nohup bash scripts/vdash/record_v5_full.sh \
#        > logs/vdash/record_v5_full.out 2>&1 &'
set -euo pipefail
cd /workspaces/isaaclab_arena
unset DISPLAY   # no-op in this container (DISPLAY already empty) but kept for parity

PY=/isaac-sim/python.sh
PARTS=datasets/vdash/_v5_full_parts
OUT=datasets/vdash/vla_pick_insert_v5_full.hdf5
LANG="Pick up the peg and insert it into the socket."
ENVARGS="vdash_pick_insert --clearance 2.0 --level L1"

rec() {  # rec <seed> <num_demos> <arm_init_std> <out.hdf5> [extra flags...]
  local seed="$1" n="$2" std="$3" out="$4"; shift 4
  $PY scripts/vdash/record_vla_demos.py --enable_cameras --num_envs 1 \
...
echo "=== MERGE ==="
$PY scripts/vdash/merge_hdf5_demos.py \
