...
Builds the vdash env with the top-down camera (needs --enable_cameras), holds the peg static for a
few steps, runs :class:`ClassicVisionGrasp`, and prints the per-env error vs the true grasp point;
saves the env-0 RGB + blue mask to docs/images/vdash/ for a visual check.

    unset DISPLAY
    /isaac-sim/python.sh scripts/vdash/test_overhead_cam.py --enable_cameras --num_envs 4 --seed 0 \\
...
_OUT = os.path.join(os.path.dirname(__file__), "..", "..", "docs", "images", "vdash")
...
def main():
    ...
    with SimulationAppContext(args_cli):
        ...
        from isaaclab_arena.controllers.vdash_classic_vision import ClassicVisionGrasp, segment_blue
        ...
        try:
            ...
            off = torch.zeros_like(peg.root_pos_w); off[:, 2] = grip_offset
            ...
            for i in range(n):
                print(f"[cam] env{i} valid={int(valid[i])} est=({est[i,0]:.3f},{est[i,1]:.3f},{est[i,2]:.3f}) "
            ...
            ax[0].imshow(rgb.astype(np.uint8)); ax[0].set_title("overhead rgb"); ax[0].axis("off")
            ax[1].imshow(mask, cmap="gray"); ax[1].set_title(f"blue mask ({int(mask.sum())} px)"); ax[1].axis("off")
            fig.tight_layout(); fig.savefig(os.path.join(_OUT, "m8_overhead_check.png"), dpi=120)
