...
# recover the in-gripper lateral error e_xy? (compares VDASH_CAL_DIAG estimate to VDASH_GRASP_DIAG GT;
...
acc() {  # off half step
  ...
  env VDASH_GRASP_OFF_MM="$off" VDASH_TACTILE_CAL=1 VDASH_CAL_DIAG=1 VDASH_GRASP_DIAG=1 \
      VDASH_CAL_HALF_MM="$half" VDASH_CAL_STEP_MM="$step" VDASH_EPISODE_S=60 \
...
cl() {  # off mode(cal|blind|oracle)
  ...
  [ "$mode" = cal ] && extra="VDASH_TACTILE_CAL=1 VDASH_EPISODE_S=60"
  [ "$mode" = oracle ] && extra="VDASH_CHEAT_TIP=1"
  env VDASH_GRASP_OFF_MM="$off" $extra \
      "$PY" scripts/vdash/run_eval_grid.py --policies vdash_scripted --clearances 2.0 --levels L1 \
