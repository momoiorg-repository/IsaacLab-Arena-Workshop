...
Pure matplotlib (no Isaac Sim):
    ...
    /isaac-sim/python.sh scripts/vdash/make_e4_figure.py \\
        --conv_csv results/vdash/e4_convergence_c2.csv \\
        --out docs/images/vdash/e4_convergence_c2.png
"""
...
def main():
    ...
    p.add_argument("--conv_csv", default=os.path.join(_REPO_ROOT, "results", "vdash", "e4_convergence_c2.csv"))
    p.add_argument("--handoff_jsonl", default=os.path.join(
        _REPO_ROOT, "logs", "vdash", "vdash_handoff_grid_vdash_scripted_c2.0_L1_*.jsonl"))
    p.add_argument("--out", default=os.path.join(_REPO_ROOT, "docs", "images", "vdash", "e4_convergence_c2.png"))
    args = p.parse_args()
    ...
    im = axL.imshow(grid, origin="lower", aspect="auto", cmap="RdYlGn", vmin=0, vmax=1,
    ...
    axL.set_xticks(rs); axL.set_yticks(ths)
    for i, th in enumerate(ths):
        for j, rr in enumerate(rs):
            if not np.isnan(grid[i, j]):
                axL.text(rr, th, f"{grid[i, j]:.0%}", ha="center", va="center", fontsize=7,
    ...
    axR.add_patch(Circle((0, 0), r_cap, facecolor="#b2f2bb", edgecolor="#2f9e44", lw=1.5,
                         alpha=0.6, label=f"convergence zone (r≤{r_cap:.0f} mm, θ≈0)"))
    ...
    if handoff is not None:
        ...
        axR.scatter(dx[~inside], dy[~inside], s=22, color="#e8590c", marker="x",
                    label=f"handoff ∉ zone ({(~inside).sum()})")
        lim = max(lim, float(np.abs(np.concatenate([dx, dy])).max()) + 1)
    axR.set_xlim(-lim, lim); axR.set_ylim(-lim, lim); axR.set_aspect("equal")
    axR.axhline(0, color="#ccc", lw=0.6); axR.axvline(0, color="#ccc", lw=0.6)
    axR.set_xlabel("handoff dx [mm]"); axR.set_ylabel("handoff dy [mm]")
