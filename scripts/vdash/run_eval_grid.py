...
"""V-DASH evaluation grid runner (brief §3 M4).

Sweeps ``policy × clearance × level`` (from ``configs/vdash/eval_grid.yaml``), running each cell
...
:class:`~isaaclab_arena.metrics.vdash_handoff_logger.VDashHandoffRecorder` is the source of truth;
...
Run inside the container:
    ...
    unset DISPLAY
    /isaac-sim/python.sh scripts/vdash/run_eval_grid.py                       # full grid
    /isaac-sim/python.sh scripts/vdash/run_eval_grid.py \\
        --clearances 2.0 1.0 --levels L1 --num_episodes 5                     # dry-run (2 cells × 5)
    /isaac-sim/python.sh scripts/vdash/run_eval_grid.py --aggregate_only      # just rebuild the CSV
"""
...
def _cell_logs(log_dir: str, policy: str, c: float, level: str) -> list[str]:
    pat = os.path.join(log_dir, f"vdash_handoff_{_run_tag(policy)}_c{_cstr(c)}_{level}_*.jsonl")
...
def aggregate(args, rows_spec) -> list[dict]:
    ...
    for policy, c, level in rows_spec:
        ...
        rec = {"policy": policy, "clearance_mm": c, "level": level, "n": 0, "success": 0,
               "success_rate": "", "ci95_low": "", "ci95_high": "",
               "insert_force_median_N": "", "handoff_fired": 0, "jsonl": os.path.basename(path) if path else ""}
        if path:
...
def write_csv(rows: list[dict], out_csv: str):
    ...
    cols = ["policy", "clearance_mm", "level", "n", "success", "success_rate", "ci95_low",
            "ci95_high", "insert_force_median_N", "handoff_fired", "jsonl"]
...
def main():
    ...
    p = argparse.ArgumentParser(parents=[pre], description="V-DASH evaluation grid runner (M4)")
    p.add_argument("--policies", nargs="+", default=cfg.get("policies", ["vdash_scripted"]))
    ...
    p.add_argument("--env", default=cfg.get("env", "vdash_pick_insert"))
    p.add_argument("--log_dir", default=cfg.get("log_dir", "logs/vdash"))
    p.add_argument("--out", dest="out_csv", default=cfg.get("out_csv", "results/vdash/eval_grid.csv"))
    ...
    print("\n[grid] summary:")
    for r in rows:
        print(f"  {r['policy']:14s} c={r['clearance_mm']:<5} {r['level']:<3} "
