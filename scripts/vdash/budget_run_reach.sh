...
# grid edge / funnel lip). Test whether a WIDER and/or FINER probe grid (VDASH_CAL_HALF_MM/STEP_MM)
...
run() {  # off_mm off_deg half step
  ...
  env VDASH_GRASP_OFF_MM="$mm" VDASH_GRASP_OFF_DEG="$dg" VDASH_TACTILE_CAL=1 \
      VDASH_CAL_HALF_MM="$half" VDASH_CAL_STEP_MM="$step" VDASH_EPISODE_S=90 VDASH_GRASP_DIAG=1 \
