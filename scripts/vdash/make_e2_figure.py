...
Reads the eval-grid CSV produced by ``scripts/vdash/run_eval_grid.py`` and plots success rate vs
...
def main():
    ...
    p.add_argument("--csv", default=os.path.join(_REPO_ROOT, "results", "vdash", "e2_clearance.csv"))
    p.add_argument("--out", default=os.path.join(_REPO_ROOT, "docs", "images", "vdash", "e2_clearance_curve.png"))
    p.add_argument("--policy", default="vdash_scripted")
    args = p.parse_args()
    ...
    for level in sorted(by_level):
        ...
        ax.errorbar(
            cs, sr, yerr=[lo, hi], marker="o", capsize=4, lw=1.8, ms=6,
            color=_LEVEL_COLOR.get(level, None), label=_LEVEL_LABEL.get(level, level),
        )
        # annotate n at each point
        for r, c, s in zip(pts, cs, sr):
            ax.annotate(f"n={int(_f(r['n']))}", (c, s), textcoords="offset points",
                        xytext=(6, -12), fontsize=7, color="#555")
