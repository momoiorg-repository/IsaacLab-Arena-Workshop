...
# in-gripper TILT injection (BDASH_GRASP_OFF_DEG, the topple condition) to find settings that let a
...
run() {  # cond off_deg extra_env
  ...
  env BDASH_GRASP_OFF_DEG="$dg" BDASH_GRASP_DIAG=1 $extra \
...
run baseline  $D ""
run rccrot03  $D "BDASH_INS_RCC_ROT_GAIN=0.03"
run rccrot08  $D "BDASH_INS_RCC_ROT_GAIN=0.08"
run rccxy     $D "BDASH_INS_RCC_XY_GAIN=0.0008"
run rccxyrot  $D "BDASH_INS_RCC_XY_GAIN=0.0008 BDASH_INS_RCC_ROT_GAIN=0.05"
run gentle    $D "BDASH_INS_F_TARGET=2.0 BDASH_INS_PRESS_FORCE_BAND=1.5 BDASH_INS_CONTACT_SPEED=0.004"
