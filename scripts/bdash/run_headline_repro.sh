#!/usr/bin/env bash
# B-DASH headline re-run from the frozen tree (commits be154b29 + 66775820).
...
# results/bdash/repro/ so they can be diffed against results/bdash/prefreeze_backup/.
set -u
cd /workspaces/isaaclab_arena
unset DISPLAY
PY=/isaac-sim/python.sh
RDIR=results/bdash/repro
LDIR=logs/bdash/repro
mkdir -p "$RDIR" "$LDIR"
M="$LDIR/master.log"
...
$PY scripts/bdash/run_eval_grid.py --policies bdash_scripted \
  --clearances 2.0 1.0 0.5 0.25 --levels L0 L1 --num_episodes 50 --num_envs 50 --seed 0 \
  --log_dir logs/bdash --no_resume --out "$RDIR/e2_clearance_final.csv" > "$LDIR/e2_grid.out" 2>&1
echo "[repro] E2 exit=$?" | tee -a "$M"
grep -E "bdash_scripted c=" "$LDIR/e2_grid.out" | tee -a "$M"
...
for MODE in recover held; do
  ...
  for S in 0 1 2 3 4; do
    $PY scripts/bdash/run_convergence_zone.py --policy_type bdash_scripted --num_envs 60 --seed "$S" $FLAG \
      --out "$RDIR/e4_${MODE}_c2.0_s${S}.csv" bdash_pick_insert --clearance 2.0 --level L0 \
...
BDASH_VLA_DEBUG=1 $PY scripts/bdash/run_eval_grid.py --policies bdash_vla_v6_recovery \
  --clearances 2.0 --levels L1 --num_episodes 20 --seed 0 \
  --log_dir logs/bdash --no_resume --out "$RDIR/eval_v6_recovery.csv" > "$LDIR/eval_v6.out" 2>&1
echo "[repro] VLA exit=$?" | tee -a "$M"
grep -E "bdash_vla_v6_recovery c=" "$LDIR/eval_v6.out" | tee -a "$M"
