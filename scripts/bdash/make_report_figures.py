...
# SPDX-License-Identifier: Apache-2.0
"""Generate the figures embedded in docs/bdash_M1-M3_report_ja.md.

Pure matplotlib/trimesh (no Isaac Sim) so it runs fast and headless:
  * peg 3D render (from assets/bdash/peg_insert/peg.stl) — shows the cylindrical grip,
  * peg+socket cross-section schematic (from configs/bdash/peg_insert/assets.yaml) — bore, funnel, clearance c,
  * clearance-series bores (c = 2.0/1.0/0.5/0.25 mm),
  * M3 success-rate progression bar chart.

Run inside the container:
    /isaac-sim/python.sh scripts/bdash/make_report_figures.py
"""
...
_ASSETS = os.path.join(_ROOT, "assets", "bdash", "peg_insert")
_OUT = os.path.join(_ROOT, "docs", "images", "bdash")
os.makedirs(_OUT, exist_ok=True)

with open(os.path.join(_ROOT, "configs", "bdash", "peg_insert", "assets.yaml")) as f:
...
def fig_cross_section():
    ...
    for poly, color, lab in ((sp, "#9aa7b5", "socket (c=2.0mm)"), (pp, "#5b8def", "peg")):
        ...
        ax.add_patch(MplPolygon(full, closed=True, facecolor=color, edgecolor="#33414f", lw=1.0, label=lab, alpha=0.85))
    # show the peg dropped into the bore (tip at depth 20mm) as a dashed overlay
    pp_in = pp.copy()
    pp_in[:, 1] += 30 - 20  # mouth at z=30, place tip at z=10
    full_in = np.vstack([pp_in, pp_in[::-1] * [-1, 1]])
    ax.add_patch(
        MplPolygon(full_in, closed=True, facecolor="none", edgecolor="#d6336c", lw=1.3, ls="--", label="peg inserted")
    )
    ax.set_xlim(-32, 32)
    ax.set_ylim(-2, 72)
    ax.set_aspect("equal")
    ...
    axz.annotate(
        ...
        xytext=(9, 33),
        fontsize=8,
        arrowprops=dict(arrowstyle="->", color="#d6336c"),
    )
    axz.annotate(
        f"clearance c\n(bore r = {peg_r:.0f}+c)",
        xy=(peg_r + 2, 18),
        xytext=(9, 12),
        fontsize=8,
        arrowprops=dict(arrowstyle="->", color="#0b7285"),
    )
    axz.set_xlim(0, 16)
    axz.set_ylim(8, 34)
    axz.set_aspect("equal")
    axz.set_xlabel("x [mm]")
    axz.set_title("mouth zoom", fontsize=10)
    fig.tight_layout()
