...
RD=results/bdash/repro; LD=logs/bdash/repro; mkdir -p "$RD" "$LD"
...
run() {  # exp off noise cheat(1|"")
  ...
  env BDASH_GRASP_OFF_MM="$off" BDASH_TIP_NOISE_MM="$noise" ${cheat:+BDASH_CHEAT_TIP=1} BDASH_GRASP_DIAG=1 \
  ...
  local kn=$(grep -E 'bdash_scripted c=' "$out" | grep -oE '[0-9]+/[0-9]+' | head -1)
