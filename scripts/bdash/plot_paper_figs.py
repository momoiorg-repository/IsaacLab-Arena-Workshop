...
"""Generate the B-DASH insertion-diagnosis figures (M11) from the CSVs / diagnostic logs on disk.

Run in the gn17 container:
    /isaac-sim/python.sh scripts/bdash/plot_paper_figs.py
Outputs -> figs/bdash/{e4_basin,ingripper,cheat,e2_clearance}.png
...
RES = os.path.join(ROOT, "results", "bdash")
LOGS = os.path.join(ROOT, "logs", "bdash")
FIGS = os.path.join(ROOT, "figs", "bdash")
...
def _basin_rows(mode):
    ...
    if seedfiles:
        rows = []
        for f in seedfiles:
            rows += list(csv.DictReader(open(f)))  # noqa: SIM115
        return rows, len(seedfiles)
    single = os.path.join(RES, "e4_recover_c2.0.csv" if mode == "recover" else "e4_convergence_c2.0.csv")
    return (list(csv.DictReader(open(single))) if os.path.exists(single) else []), 1  # noqa: SIM115
...
def fig_e4_basin():
    ...
    for ax, rows, ncell, title in (
        (axes[0], held_rows, nh, "Held (static tolerance)"),
            ...
            for j, rr in enumerate(rs):
                if not np.isnan(g[i, j]):
                    ax.text(
                        ...
                        ha="center",
                        va="center",
                        color="w" if g[i, j] < 55 else "k",
                        fontsize=7,
                    )
    fig.colorbar(im, ax=axes, label="insert success (%)", shrink=0.8)
    fig.suptitle(
        "E4 convergence basin (bdash_scripted, c=2.0, L0 — deterministic; each cell = mean "
        "over tilt azimuths)\nthe controller recovers offset to 5 mm & tilt to ~8°",
        fontsize=10,
    )
...
def _parse_ingrip(logpath, pol):
    if not os.path.exists(logpath):
        return [], []
    txt = open(logpath, errors="ignore").read()  # noqa: SIM115
    tilt, lat = [], []
...
def fig_e2():
    ...
    if not os.path.exists(path):
        ...
        return
    rows = [r for r in csv.DictReader(open(path)) if r.get("success_rate") not in (None, "")]  # noqa: SIM115
    if not rows:
    ...
    for level, color in (("L0", "#999999"), ("L1", "#4f81bd")):
        pts = sorted([
            (
                float(r["clearance_mm"]),
                100 * float(r["success_rate"]),
                100 * float(r["ci95_low"]),
                100 * float(r["ci95_high"]),
            )
            for r in rows
            if r["level"] == level
        ])
        ...
        ax.errorbar(
            c,
            sr,
            yerr=[np.array(sr) - np.array(lo), np.array(hi) - np.array(sr)],
            marker="o",
            capsize=3,
            color=color,
            label=level,
    ...
    ax.set_title("E2 clearance curve (bdash_scripted, n=50, Wilson 95%)", fontsize=10)
    ax.set_ylim(0, 105)
    ax.invert_xaxis()
    ax.legend()
