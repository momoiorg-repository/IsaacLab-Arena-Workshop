...
# SPDX-License-Identifier: Apache-2.0
"""Capture Isaac Sim camera frames of the B-DASH pick→insert for the report.

Builds the ``bdash_pick_insert`` env (like ``policy_runner``), drives it with the ``bdash_scripted``
...
``--enable_cameras``):
    ...
    unset DISPLAY
    /isaac-sim/python.sh scripts/bdash/capture_views.py --policy_type bdash_scripted \
...
def main():
    ...
    parser.add_argument("--out_dir", type=str, default="docs/images/bdash")
    ...
    with SimulationAppContext(args_cli):
        ...
        from isaaclab_arena.policy.bdash_scripted_policy import BDashScriptedPolicy, BDashScriptedPolicyArgs
        ...
        policy = BDashScriptedPolicy(BDashScriptedPolicyArgs())
        obs, _ = env.reset()
        policy.reset()
