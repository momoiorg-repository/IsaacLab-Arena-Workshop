...
def main():
    ...
    while recorded < args_cli.num_demos and simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            kicked = False
            prevcam = None  # BDASH_REC_DBG: detect whether the live camera obs actually changes step-to-step
            for step in range(max_steps):
                ...
                if os.environ.get("BDASH_REC_DBG"):
                    cam = env.obs_buf["camera_obs"]["wrist_cam_rgb"][0].detach().float()
                    if prevcam is not None and step % 15 == 1:
                        print(
                            f"[recdbg] step={step} wrist obs framediff={(cam - prevcam).abs().mean():.4f}", flush=True
                        )
                    prevcam = cam.clone()
                if cut():
                    reached = True
