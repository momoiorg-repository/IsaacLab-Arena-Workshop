#!/usr/bin/env bash
# Validate the §2.1 EE de-cant (tilt fix) on the scripted tilt test-bed (BDASH_GRASP_OFF_DEG induces
...
cl() {  # off_deg mode
  ...
  [ "$mode" = decant ] && extra="BDASH_DECANT=1 BDASH_DECANT_GAIN=$GAIN BDASH_DECANT_SIGN=$SIGN BDASH_CAL_DIAG=1 BDASH_EPISODE_S=40"
  [ "$mode" = full ]   && extra="BDASH_DECANT=1 BDASH_DECANT_GAIN=$GAIN BDASH_DECANT_SIGN=$SIGN BDASH_TACTILE_CAL=1 BDASH_EPISODE_S=80"
  [ "$mode" = oracle ] && extra="BDASH_CHEAT_TIP=1"
  env BDASH_GRASP_OFF_DEG="$d" BDASH_GRASP_DIAG=1 $extra \
