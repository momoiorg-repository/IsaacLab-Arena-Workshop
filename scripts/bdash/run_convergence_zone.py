...
Run inside the container:
    ...
    unset DISPLAY
    /isaac-sim/python.sh scripts/bdash/run_convergence_zone.py \\
        --policy_type bdash_scripted --num_envs 16 --seed 0 \\
        --r_mm 0 1 2 3 4 5 --az_deg 0 90 180 270 --tilt_deg 0 2 4 6 8 \\
        --out results/bdash/e4_convergence_c2.csv \\
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "recovery basin: deliver the peg to (r,θ) then release so the controller "
            "recenters on the true axis (vs the default held static-tolerance map)"
        ),
    )
    parser.add_argument("--out", default=os.path.join(_REPO_ROOT, "results", "bdash", "e4_convergence.csv"))
    ...
    with SimulationAppContext(args_cli):
        ...
        with open(args_cli.out, "w", newline="") as fh:
            w = csv.DictWriter(
                fh, fieldnames=["clearance_mm", "r_mm", "az_deg", "tilt_deg", "dx_mm", "dy_mm", "success"]
            )
            w.writeheader()
            w.writerows(results)
