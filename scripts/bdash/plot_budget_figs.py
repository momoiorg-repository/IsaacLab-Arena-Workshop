...
"""Precision-budget analysis + figures for B-DASH (docs/bdash_precision_budget_plan.md).

Consumes the scripted-test-bed sweeps (``results/bdash/repro/sweeps.csv``) and the VLA in-gripper
grasp distribution (``results/bdash/repro/grasp_diag_vla_v6.csv``) and produces:
...
def fig_widening():
    ...
    Parsed from logs/bdash/repro/probe.log ('[probe] cl off=X mode: k/n')."""
    import re

    p = os.path.join(REPO, "logs", "bdash", "repro", "probe.log")
    if not os.path.exists(p):
        return
    data = {}
...
def fig_decant():
    ...
    injected tilt. Parsed from logs/bdash/repro/decant.log ('[decant] off_deg=X mode: k/n')."""
    import re

    p = os.path.join(REPO, "logs", "bdash", "repro", "decant.log")
    if not os.path.exists(p):
        return
    data = {}
    for ln in _readlines(p):
