...
Examples::
    ...
    docker exec isaaclab_arena-latest bash -lc '
      ...
      /isaac-sim/python.sh scripts/bdash/m1_insertion_stability.py --clearance 0.25 --headless'

    # watch in the GUI via WebRTC (An's preferred inspection)
    docker exec isaaclab_arena-latest bash -lc '
      cd /workspaces/isaaclab_arena && unset DISPLAY &&
      /isaac-sim/python.sh scripts/bdash/m1_insertion_stability.py --clearance 0.5 --livestream 2'
"""
...
parser = argparse.ArgumentParser(description="B-DASH M1 peg-insertion stability check.")
parser.add_argument("--clearance", type=float, default=0.25, help="Radial clearance c (mm).")
parser.add_argument("--assets-dir", type=str, default="assets/bdash/peg_insert")
parser.add_argument(
    "--offsets-frac",
    type=float,
    nargs="*",
...
parser.add_argument(
    ...
    default=0.5,
    help=(
        "Effective held-peg mass (kg). Stands in for the arm's inertia so the "
...
parser.add_argument("--out", type=str, default="output/bdash/m1_stability.json")
...
# Geometry (must match configs/bdash/peg_insert/assets.yaml)
SHAFT_R = 0.004
SOCKET_H = 0.030
BORE_DEPTH = 0.025
...
def main():
    ...
    for i, frac in enumerate(offsets):
        rows.append({
            ...
            "inserted": bool(inserted[i]),
            "exploded": bool(exploded[i]),
    ...
    result = {
        "clearance_mm": c_mm,
        "bore_radius_mm": round(bore_r * 1000, 3),
        "descent_speed_mps": args_cli.descent_speed,
        "peg_mass_kg": args_cli.peg_mass,
