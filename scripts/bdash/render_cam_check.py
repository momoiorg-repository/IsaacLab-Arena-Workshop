...
``{out_prefix}_{wrist,left,right}.png`` to ``--out_dir``.
    ...
    unset DISPLAY
    /isaac-sim/python.sh scripts/bdash/render_cam_check.py --enable_cameras --num_envs 1 --seed 0 \\
        --focal 2.8 --out_prefix before bdash_pick_insert --clearance 2.0 --level L1
"""
...
parser.add_argument("--out_dir", type=str, default="logs/bdash/camcheck")
...
def main():
    ...
    with torch.inference_mode():
        for i in range(args_cli.n_resets):
            ...
            # Best-effort: only the bdash task exposes a "peg" name. Other scenes (e.g. bdash) are
