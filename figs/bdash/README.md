# B-DASH insertion-diagnosis figures (M11)

Regenerate all: `/isaac-sim/python.sh scripts/bdash/plot_paper_figs.py` (gn17 container).
Every number traces to a CSV/JSONL on disk (see the paper-claims audit `share/bdash_paper_claims_audit.md`).
...
|---|---|---|
| `ingripper.png` | The VLA holds the peg cocked **in the gripper** (8°/9 mm) vs scripted 0.2°/0.8 mm — the root cause of the 20% insert. | `logs/bdash/graspdiag_{scripted,vla}.out` (`BDASH_GRASP_DIAG`) |
| `cheat.png` | The failure **is** the tip-estimate error: nominal 98% → 8 mm in-gripper offset 0% → give the controller the true tip 55%. Even ideal tip knowledge caps ~55% (peg still held off-axis). | `logs/bdash/cock8_{test,cheat}.out` |
| `e4_basin.png` | The controller is **robust to handoff pose** — recovery basin ~84% (offset to 5 mm, tilt to ~8°) vs held ~39%. So neither handoff offset nor tilt explains 20%. (L0 scene is fixed → the basin is **deterministic**; each cell = mean over tilt azimuths.) | `results/bdash/e4_{held,recover}_c2.0_s*.csv` |
| `e2_clearance.png` | Classical insertion success vs radial clearance (scripted, n=50, Wilson 95%), L0 & L1. | `results/bdash/e2_clearance_final.csv` |
