# B-DASH — consolidated status (single-page overview)

**Updated:** 2026-06-26 · **Branch:** `an/bdash-vla-training` · **Targets:** figure freeze 7/2, paper 7/9
This is the one-page "全貌": the story, the verified results, what we can/can't claim, open decisions,
and where every artifact lives. Numbers are audit-verified (`share/bdash_paper_claims_audit.md`); all
...
### 3. Classical control is not the bottleneck — `M11.md §2-3`, `figs/bdash/e4_basin.png`
...
### 4. Residual = grasp quality (in-gripper) — `M11.md §4`, `figs/bdash/ingripper.png`
...
### 5. Unfixable downstream (§2.1 limiter) — `M11.md §5`, `figs/bdash/cheat.png`
...
### 6. E2 clearance cliff — `figs/bdash/e2_clearance.png`
...
1. **Reproducibility (audit #1) — RESOLVED 2026-06-27:** the headline code+config is frozen on
   ...
   **c2.0 L0 50/50, L1 49/50; c≤1.0 all 0/50** (bit-identical to the prior run, incl. `f_ins` to 3 dp),
   ...
   `scripts/bdash/run_headline_repro.sh` committed; the numbers are now load-bearing.
...
- `docs/milestones/B-DASH_next_actions_M10.md` — the action plan this work executed
- `share/bdash_paper_claims_audit.md` — every number traced to source, discrepancies flagged
- `docs/physics_params.md` — physics provenance (contact_offset reconciled to 0.0002)

**Figures** — `figs/bdash/` (`README.md` = index + headline numbers)
- `ingripper.png` (grasp quality) · `cheat.png` (tip-error proof) · `e4_basin.png` (held vs recovery) ·
  `e2_clearance.png` (clearance cliff). Regenerate: `scripts/bdash/plot_paper_figs.py`.

**Live data** — `results/bdash/`
- `eval_grid.csv` (scripted 98% headline) · `e2_clearance_final.csv` (E2 sweep) ·
  `e4_{held,recover}_c2.0_s0..s4.csv` (basin) · VLA eval JSONL
  `logs/bdash/bdash_handoff_grid_bdash_vla_v6_recovery_c2.0_L1_20260625_233557.jsonl`
- In-gripper diagnostic logs: `logs/bdash/graspdiag_{scripted,vla}.out`
- Cheat/test-bed logs: `logs/bdash/cock8_{test,cheat}.out`
...
- `isaaclab_arena/policy/{bdash_vla_policy,bdash_scripted_policy}.py` — `BDASH_GRASP_DIAG` instrument
- `isaaclab_arena/evaluation/policy_runner.py` & `scripts/bdash/record_vla_demos.py` — camera fix
- `scripts/bdash/{run_eval_grid,run_convergence_zone,plot_paper_figs,train_and_push_v6_recovery}.py`
...
`~/.claude/projects/-home-an-Workspace/memory/bdash-vla-training-pipeline.md` (complete timeline v3→v6
+ diagnosis + finalization).

---

## Housekeeping (stale / duplicate data — safe to ignore or prune)
- `results/bdash/e4_convergence_c3.0.csv` — **mislabeled**, is actually c2.0 (clearance snapped).
...
- Paused/shelved: full pick→insert recording (`datasets/bdash/_v5_full_parts/full_clean.hdf5`, 500-demo
