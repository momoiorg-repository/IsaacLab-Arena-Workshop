# V-DASH recovery / diversity data plan (v5-recovery)
...
The diagnostics (gated `VDASH_VLA_DEBUG` geometry probe, same seed-0 pegs each time) say the bottleneck
...
`scripts/vdash/record_vla_demos.py` today only perturbs the *start* (`--arm_init_std`, applied via the
...
Record into one HDF5, `datasets/vdash/vla_pick_handoff_v5_recovery.hdf5`, L1 / c2.0, seed 0:
...
1. Converter config `isaaclab_arena_gr00t/lerobot/config/vdash_pick_handoff_v5_recovery_config.yaml`
   (clone of `…_v4_config.yaml`, `hdf5_name → vla_pick_handoff_v5_recovery.hdf5`, output
   `datasets/vdash/vla_pick_handoff_v5_recovery/lerobot`). Convert in the **gn17** container.
2. Train `nvidia/GR00T-N1.7-3B` on the cloud A100/H100 (won't fit the 5080):
   ...
   DATASET_PATH=datasets/vdash/vla_pick_handoff_v5_recovery/lerobot \
   OUTPUT_DIR=models/vdash-gr00t-n1-7-pick-handoff-v5-recovery \
...
Pull each saved checkpoint, wire `vdash_vla_v5_recovery` + config into `run_eval_grid.py` (clone the v4
...
- **Flat (~0):** the in-recorder perturbation isn't covering the real eval states → escalate to **full
  closed-loop DAgger**: roll out the *actual VLA* (`vdash_vla_v4`), log the states it visits, relabel
...
- [x] **Camera fix** — side-cam `focal_length` 2.8→8 in `franka.py`; verified L1 extremes stay framed,
      peg ~3px→~12–15px (`logs/vdash/camcheck/`). 2026-06-25.
...
- [x] **Recorded the 1200-demo mixed set** (600 clean 100% / 300 wide 100% / 300 recovery 95%) +
      merged → `datasets/vdash/vla_pick_handoff_v5_recovery.hdf5` (5.0GB, total_steps 221574). Verified:
      kick spike present only in the recovery region (0.135 vs 0.013 rad), keys consistent. 2026-06-25.
- [x] **Converted + pushed**: `vdash_pick_handoff_v5_recovery_config.yaml` → gn17 convert → 1200 parquet
