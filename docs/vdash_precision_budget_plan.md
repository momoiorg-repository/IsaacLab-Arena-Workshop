# V-DASH: precision-budget theory + §2.1-honest interface-widening — research plan
...
M11 localized the V-DASH end-to-end ceiling (20%, 4/20) to the VLA's **in-gripper grasp error** (peg
...
- **Correctable channel** — error that *enters* `O` and is servoed out; tolerance = the measured
  convergence basin `B(r,θ)` (E4). For V-DASH: handoff-delivery error `d=(r,θ_h)` (assumed-tip vs known
  socket) is correctable.
- **Blind channel** — error *hidden* by `O`, surviving as a fixed bias; tolerance = the measured blind
  response `R(g)`. For V-DASH: in-gripper grasp `g=(l,φ)` is blind (`tip_estimate` assumes `g=0`,
...
- **Secondary: onboard wrist-cam estimate** (reuse `vdash_classic_vision` segment+back-project) — but the
...
- **RUN 0 (code):** re-add `VDASH_GRASP_OFF_MM` (+`_DEG`) test-bed in `scripted_pick.py`; unify
  `tip_correction` + `VDASH_CHEAT_TIP` oracle in `insertion_controller.py`. *(Phase 1, this commit.)*
- **RUN 1 — blind response `R(g)`:** scripted, `VDASH_GRASP_OFF_MM/_DEG` sweep l∈{0,2,4,6,8,10,12} ×
  φ∈{0,2,4,6,8,10}, c∈{1,2,3}, n≥20, batched, no cameras → `blind_response_c*.csv`.
- **RUN 2 — oracle `R_oracle(g)`:** same sweep + `VDASH_CHEAT_TIP` → `oracle_response_c2.0.csv`.
- **RUN 3 — VLA `p_grasp(g)`:** `VDASH_GRASP_DIAG` over `vdash_vla_v6_recovery`, n≥40, ≥3 seeds → parse to
...
- **Probe may not recover lateral at 9–17 mm** → lead with bore-edge bisection (not cone), run at REAL
  magnitudes first, report `e_xy` RMSE vs `VDASH_GRASP_DIAG`; scope to c≥2.0.
...
- **§2.1 leak-by-statistics** (magnitude priors, GT-tuned thresholds) → data-adaptive search, freeze
  hyperparameters before eval, ship a grep-audit; `VDASH_GRASP_DIAG` is scoring-only.
...
All §2.1-gated probes default OFF (headline path unchanged). Figures: `figs/vdash/budget_*.png`
(`plot_budget_figs.py`); data `results/vdash/repro/{sweeps,grasp_diag_vla_v6,probe_*,dc_*}.csv`.

1. **Theory predicts the VLA (non-circular).** From independent measurements (R from the test-bed,
   p_grasp from VDASH_GRASP_DIAG — neither sees the insert), P_pred = 5.4% (effective err e=lat+60·sinθ)
