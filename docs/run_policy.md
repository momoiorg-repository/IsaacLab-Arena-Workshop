# B-DASH — How to Run a Policy (cheatsheet)

Closed-loop evaluation of policies in the `bdash_pick_insert` task.
...
Outputs: per-episode JSONL in `logs/bdash/` (source of truth), aggregated CSV in `results/bdash/eval_grid.csv`.
...
/isaac-sim/python.sh scripts/bdash/run_eval_grid.py \
  --policies bdash_scripted --clearances 2.0 --levels L1 --num_episodes 5
```

### 2. VLA hierarchy — v2 handoff model (local)

VLA does pick→handoff, then the rule-based `InsertionController` inserts. Uses `models/bdash-gr00t-n1-6-pick-handoff`.

```bash
/isaac-sim/python.sh scripts/bdash/run_eval_grid.py \
  --policies bdash_vla --clearances 2.0 --levels L1 --num_episodes 5
```
...
/isaac-sim/python.sh scripts/bdash/run_eval_grid.py --policies bdash_scripted

# Multiple policies + custom cells
/isaac-sim/python.sh scripts/bdash/run_eval_grid.py \
  --policies bdash_scripted bdash_vla --clearances 2.0 1.0 --levels L1 --num_episodes 20

# Re-aggregate CSV from existing logs (no sim re-run)
/isaac-sim/python.sh scripts/bdash/run_eval_grid.py \
  --policies bdash_vla --clearances 2.0 --levels L1 --num_episodes 5 --aggregate_only

# Force re-run even if a complete JSONL exists
/isaac-sim/python.sh scripts/bdash/run_eval_grid.py ... --no_resume
```
...
LIVESTREAM=2 /isaac-sim/python.sh isaaclab_arena/evaluation/policy_runner.py \
  ...
  bdash_pick_insert --clearance 2.0 --level L1 --embodiment franka_joint
```
...
hf download umegan/vdash-gr00t-n1-6-pick-handoff-v3 \
  --local-dir models/bdash-gr00t-n1-6-pick-handoff-v3

# point model_path (line 16) at the v3 dir, then run the §2 bdash_vla command:
sed -i 's#models/bdash-gr00t-n1-6-pick-handoff$#models/bdash-gr00t-n1-6-pick-handoff-v3#' \
...
✅ Already installed: model at `models/bdash-gr00t-n1-6-pick-insert-full/`, config at
`isaaclab_arena_gr00t/policy/config/bdash_pick_insert_full_gr00t_closedloop_config.yaml`.
...
/isaac-sim/python.sh isaaclab_arena/evaluation/policy_runner.py \
  ...
  --policy_config_yaml_path isaaclab_arena_gr00t/policy/config/bdash_pick_insert_full_gr00t_closedloop_config.yaml \
  --enable_cameras --num_episodes 5 --num_envs 1 --seed 0 \
  --log_dir logs/bdash --run_tag insertfull \
  bdash_pick_insert --clearance 2.0 --level L1 --embodiment franka_joint
```
...
hf download umegan/vdash-gr00t-n1-6-pick-insert-full \
  --local-dir models/bdash-gr00t-n1-6-pick-insert-full
sed -e 's#models/bdash-gr00t-n1-6-pick-handoff#models/bdash-gr00t-n1-6-pick-insert-full#' \
...
|---|---|---|
| `bdash-gr00t-n1-6-pick-handoff` (v2) | local + HF | `bdash_vla` (hierarchy) |
| `bdash-gr00t-n1-6-pick-handoff-v3` | HF (⚠️ empty, no weights) | `bdash_vla` (hierarchy) |
| `bdash-gr00t-n1-6-pick-insert-full` | local + HF | `Gr00tClosedloopPolicy` (end-to-end) |
| *(none)* — `bdash_scripted` | rule-based baseline | `bdash_scripted` |

## Gotchas

- **Root-owned `results/bdash/`** → CSV write crashes *after* episodes run (JSONL is safe). Fix from the host:
  `docker exec isaaclab_arena-cuda_gr00t_gn16 chown -R an:an /workspaces/isaaclab_arena/results/bdash`
