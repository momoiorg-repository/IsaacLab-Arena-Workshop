# V-DASH 触覚プローブ（bore-edge tactile probe）— 設計ノート
...
全体計画は `docs/vdash_precision_budget_plan.md` を参照。

## 1. 背景と目的

M11 で、V-DASH の挿入成功率を 20%（4/20）に縛っているのは VLA の **グリッパ内把持誤差**（ペグが
...
`VDASH_CHEAT_TIP` のみがゲート内で真姿勢を使う＝§2.1 違反の上限値専用）。
...
| 環境変数 | 既定 | 意味 |
|---|---|---|
| `VDASH_TACTILE_CAL` | off | プローブ有効化 |
| `VDASH_CAL_HALF_MM` | 12 | グリッド半幅（mm） |
| `VDASH_CAL_STEP_MM` | 6 | グリッド間隔（mm）→ 既定 5×5=25 タッチ |
| `VDASH_CAL_DEPTH_MM` | 8 | 探索フロア（口元からの最大下降, mm） |
| `VDASH_CAL_FTOUCH_N` | 2.0 | 接触判定（ベースライン超過, N） |
| `VDASH_CAL_PENMARGIN_MM` | 2.0 | funnel リップノイズの足切り（mm） |
| `VDASH_CAL_DIAG` | off | env0 の推定 `e` をログ |

関連診断：`VDASH_GRASP_DIAG`（真の `e` を eval 専用で記録＝採点用）、`VDASH_GRASP_OFF_MM`
（テストベッドで把持を意図的にずらす）、`VDASH_CHEAT_TIP`/`VDASH_TIP_NOISE_MM`（上限と精度仕様）。

## 6. 検証手順（GPU 解放後）

1. **精度:** `VDASH_GRASP_OFF_MM` を 0〜12mm で振り、`VDASH_TACTILE_CAL=1 VDASH_CAL_DIAG=1` で
   推定 `e_xy` を `VDASH_GRASP_DIAG`（真値）と比較 → **`e_xy` RMSE** を報告（メイク・オア・ブレイク）。
2. **精度仕様:** `R_widened(σ)` 掃引（`VDASH_TIP_NOISE_MM`）から「何 mm の RMSE で何 % か」を読み、
   プローブのグリッド解像度（タッチ数）を調整。
3. **閉ループ:** scripted テストベッドで `VDASH_TACTILE_CAL=1`、offset 8mm → 0/20 がどこまで回復するか
   （目標：チート 55% に肉薄）。
4. **回帰:** `VDASH_TACTILE_CAL` 無効で scripted が 98% のまま（既定パス不変）を確認。
5. **VLA:** `vdash_vla_v6_recovery` に適用し、20% → どこまで上がるか（再学習なし）。
