...
- `configs/vdash/assets.yaml`: 全寸法・クリアランス系列・collision パラメータを外出し。
- `isaaclab_arena_environments/mdp/env_callbacks.py`: Forge 物理コールバック
  `vdash_assembly_env_cfg_callback`（dt=1/120, TGS solver, max_position_iteration_count=192,
...
| ファイル | 役割 |
|---|---|
| `mdp/vdash_predicates.py` | §3.4 述語: `grasped` / `handoff` / `inserted` / `force_violation` / `insertion_failed`。torch のみ・sim 非依存。 |
| `tasks/vdash_pick_insert_task.py` | socket=固定(kinematic)・peg=held。終了条件 `success=inserted` / `insertion_failed=力超過` / `object_dropped` / `time_out`。 |
| `metrics/vdash_handoff_logger.py` | `RecorderTerm` でステップ毎に状態蓄積→reset 前に 1 エピソード 1 行の JSONL を書く。 |
| `mdp/vdash_randomization.py` | シーン多様性 L0–L3 プロファイル（姿勢／照明／テクスチャ／ディストラクタ）。 |
| `vdash_pick_insert_environment.py` + `cli.py` | 環境合成と登録（`--clearance`/`--level`/`--preinsert`）。 |
...
- **scene キーは登録アセット名**（`vdash_peg`, `vdash_socket_c1`…）でビルド毎に変わるため、
...
### 結果（実機検証・ヘッドレス）
- 述語ユニットテスト合格（`VDASH_PREDICATES_OK`）。
...
- `policy/vdash_scripted_policy.py`: pick → insertion を逐次接続（pick 完了で insertion にラッチ＝ハンドオフ）。
  policy `vdash_scripted` として登録。
- `configs/vdash/controllers.yaml`: ゲイン・閾値を全て外出し。
...
1. **アームが遅すぎる（~0.04 m/s）** → 1 エピソードが時間内に終わらない。
   ...
   対策: V-DASH 専用 cfg（`FRANKA_PANDA_VDASH_CFG`, stiffness 400 + 接触センサ）。コンプライアンスは
...
![M3 成功率の推移](images/vdash/m3_success_progression.png)
*L1 / c=2.0mm / 50試行での成功率推移。74% がこれまでの最高。目標 80%（破線）に未達。*

![peg 3D](images/vdash/peg_3d.png)
...
docker exec -it isaaclab_arena-latest bash -lc '
  ...
  LIVESTREAM=2 /isaac-sim/python.sh isaaclab_arena/evaluation/policy_runner.py \
    --policy_type vdash_scripted --num_steps 900 --headless --livestream 2 --num_envs 1 \
...
docker exec isaaclab_arena-latest bash -lc 'cd /workspaces/isaaclab_arena && unset DISPLAY && \
  /isaac-sim/python.sh scripts/vdash/generate_assets.py --headless'        # peg + socket 全て
# 一部だけ再生成（高速）: peg のみ / socket のみ
#   /isaac-sim/python.sh scripts/vdash/generate_assets.py --peg-only   --headless
#   /isaac-sim/python.sh scripts/vdash/generate_assets.py --socket-only --headless
```
出力: `assets/vdash/{peg,socket_c*}.usd`（root 所有。再生成時はコンテナ内で旧ファイル削除）。

### ③ M2: 述語・ログの検証
```bash
# 述語ユニットテスト（Isaac 不要・数秒）
docker exec isaaclab_arena-latest bash -lc 'cd /workspaces/isaaclab_arena && \
  /isaac-sim/python.sh isaaclab_arena_environments/tests/test_vdash_predicates.py'   # -> VDASH_PREDICATES_OK

# 成功経路 + §3.5 ログ出力（preinsert で inserted→success を確認）
docker exec isaaclab_arena-latest bash -lc 'cd /workspaces/isaaclab_arena && unset DISPLAY && \
  /isaac-sim/python.sh isaaclab_arena/evaluation/policy_runner.py \
    --policy_type zero_action --num_steps 60 --headless \
    vdash_pick_insert --clearance 2.0 --level L0 --preinsert --run_tag demo'
# ログ: logs/vdash/vdash_handoff_*.jsonl（1 エピソード 1 行）
```
...
docker exec isaaclab_arena-latest bash -lc 'cd /workspaces/isaaclab_arena && unset DISPLAY && \
  /isaac-sim/python.sh isaaclab_arena/evaluation/policy_runner.py \
    --policy_type vdash_scripted --num_episodes 50 --headless --num_envs 8 --seed 0 \
    vdash_pick_insert --clearance 2.0 --level L1 --run_tag eval'
# 成功率は logs/vdash/vdash_handoff_eval_*.jsonl の result 集計で確認。
# ステップ毎の局面・力・傾きを見たい時は環境変数 VDASH_DEBUG=1 を付与。
```
...
```bash
docker exec isaaclab_arena-latest bash -lc 'cd /workspaces/isaaclab_arena && unset DISPLAY && \
  /isaac-sim/python.sh scripts/vdash/make_report_figures.py'   # -> docs/images/vdash/*.png
```
...
## 成果物（ファイル）
- アセット: `scripts/vdash/generate_assets.py`, `configs/vdash/assets.yaml`, `assets/vdash/*.usd`
- タスク/述語/ログ: `isaaclab_arena/tasks/vdash_pick_insert_task.py`,
  `isaaclab_arena_environments/mdp/vdash_predicates.py`, `isaaclab_arena/metrics/vdash_handoff_logger.py`
- コントローラ: `isaaclab_arena/controllers/{ee_control,scripted_pick,insertion_controller}.py`,
  `isaaclab_arena/policy/vdash_scripted_policy.py`, `configs/vdash/controllers.yaml`
- 環境/設定: `isaaclab_arena_environments/vdash_pick_insert_environment.py`,
  `configs/vdash/task.yaml`, `mdp/vdash_config.py`, `mdp/vdash_randomization.py`
