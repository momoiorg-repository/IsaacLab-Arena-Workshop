...
The same GR00T N1.5 policy can also be evaluated as a *remote policy*
...
If the models directory was already configured earlier (for example via
the ``MODELS_DIR`` environment variable), the ``-m`` flag can be
omitted. Otherwise, use ``-m`` to point to the directory that contains
the GR00T model checkpoint.
