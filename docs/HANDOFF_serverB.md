# V-DASH — Server B continuation handoff

You (Server B: GPU + Isaac Sim + GR00T + fine-tune capable) are taking over the V-DASH compute work
while another machine writes the paper. This briefs you on the current state and the next lever.

## 0. Update the repo first
Repo: `momoiorg-repository/IsaacLab-Arena-Workshop`, branch `an/vdash-vla-training`.
```
git fetch origin && git checkout an/vdash-vla-training && git pull --ff-only origin an/vdash-vla-training
git lfs pull   # figs/vdash/*.png are LFS
```
Your local copy was old — after this you have all the V-DASH code + docs (commits be154b29, 66775820,
...
de-cant ceiling; see `figs/vdash/budget_*.png`). v6 was trained on the camera-fixed v5-recovery set
...
The budget gives the acceptance bar: drive measured in-gripper tilt/lat (VDASH_GRASP_DIAG) under the
spec and the existing classical controller should convert most handoffs.

## 3. Key files
- Docs: `docs/vdash_precision_budget_plan.md` (theory + run matrix + Results), `docs/milestones/vdash_status.md`
  (single-page overview), `M10.md`, `M11.md`, `share/vdash_paper_claims_audit.md`, `docs/run_policy.md`
  (run cheatsheet), `docs/V-DASH_dev_brief_v2.md` (design + §2.1 contract).
- Code (env-gated): `isaaclab_arena/controllers/insertion_controller.py` (CALIBRATE bore-edge probe,
  `_apply_decant`, `tip_correction`, `VDASH_CHEAT_TIP`, `VDASH_TIP_NOISE_MM`),
  `controllers/scripted_pick.py` (`VDASH_GRASP_OFF_MM`/`_DEG` test-bed),
  `policy/vdash_scripted_policy.py` (`VDASH_GRASP_DIAG`/`VDASH_FINGER_DIAG`).
- Scripts: `scripts/vdash/{plot_budget_figs.py, budget_run_*.sh, run_headline_repro.sh, run_eval_grid.py,
...
- **§2.1**: the insertion controller may use ONLY ee_frame proprioception, the fixed socket pose, wrist
  F/T, and per-finger contact forces — NEVER the peg's true pose. `VDASH_CHEAT_TIP` violates this
...
- Training: 3B fp32 fine-tune OOMs a 16GB card → use B's A100/H100-class GPU; gn17 container = N1.7
  (transformers 4.57.3).
- Reproduce the frozen headline: `scripts/vdash/run_headline_repro.sh` (~1h, gn17).
...
(`vdash-vla-training-pipeline.md`). If you need more than this brief, ask the user to paste it (or it may
