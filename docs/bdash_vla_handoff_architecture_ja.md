# B-DASH システムアーキテクチャ — VLA → ルールベース ハンドオフ（M6 スイッチャ）

対象: B-DASH「VLA（知覚・把持・搬送）＋ ルールベース（精密嵌合）」階層制御の **引き継ぎ
（handoff）機構**。本ドキュメントは「現状の B-DASH 実装でハンドオフがどう動くか」「VLA から
...
| ドキュメント | 内容 |
|---|---|
| `docs/B-DASH_dev_brief_v2.md` | 設計指針（§2.1 入力制約 / §3.3 コントローラ / §3.4 述語 / §3.5 ロガー） |
...
| `docs/milestones/M8.md` | 古典ビジョン把持ベースライン（`bdash_rule_based`） |
| `docs/milestones/M9.md` | VLA デモ収集 + インターフェイス凍結（教師データ生成） |
| `docs/bdash_M1-M3_report_ja.md` | M1〜M3 の実装記録（日本語） |
...
```
                ┌──────────────────────────────────────────────────────────┐
                │                  BDashVLAPolicy  (policy "bdash_vla")      │
                │              isaaclab_arena/policy/bdash_vla_policy.py     │
                │                                                            │
   camera RGB   │   ┌─────────────────┐                                     │
...
### 3つの B-DASH ポリシー（前段の違い・後段は共有）

| ポリシー名 | 前段（ピック・搬送） | 後段（嵌合） | 切り替えトリガ | 用途・関連 |
|---|---|---|---|---|
| `bdash_scripted` | スクリプト専門家（特権・真値参照） | `InsertionController` | ピックの `finished` フラグ | 教師データ生成（M9）・成功曲線（M5） |
| `bdash_rule_based` | スクリプト専門家＋古典ビジョン把持 | `InsertionController` | ピックの `finished` フラグ | ルールベース単体ベースライン（M8） |
| **`bdash_vla`** | **GR00T VLA（学習）** | `InsertionController` | **§3.4 `handoff` 述語** | **VLA 評価（M6・本書の主題）** |
...
`isaaclab_arena_environments/mdp/bdash_peg_predicates.py` の `handoff()`。
...
### 閾値（`configs/bdash/peg_insert/task.yaml` に外出し）
...
「ハンドオフ」という語は B-DASH では2つの文脈で使われる。混同しないこと。

### 3.1 データ記録時のハンドオフ（学習データの切り出し点・M9）

`scripts/bdash/record_vla_demos.py`。教師データの収集では（詳細は `docs/milestones/M9.md`）：

- 特権スクリプト専門家（`bdash_scripted`）がアームを駆動（§2.1 はデモ生成での専門家使用を許可）。
...
for _ in range(max_steps):
    action = policy.get_action(env, None)   # bdash_scripted が駆動
    env.step(action)
...
`bdash_vla_policy.py` の実行時。本ドキュメントの主題。次節で詳説する。

---

## 4. VLA → ルールベースへの切り替え機構（`BDashVLAPolicy.get_action`）

中心は `isaaclab_arena/policy/bdash_vla_policy.py:190` の `get_action`。

```python
def get_action(self, env, observation):
    self._ensure(env)                                  # 初回のみ後段・IK・述語パラメータを構築
    import isaaclab_arena_environments.mdp.bdash_peg_predicates as vp
    uenv = env.unwrapped
...
### 4.2 なぜ「述語」で切り替えるのか（`bdash_scripted` との対比）

| | トリガ | 取得元 |
|---|---|---|
| `bdash_scripted` | `pick.step()` が返す `finished` フラグ | **ポリシー内部の状態機械**（TRANSPORT→DONE 到達） |
| `bdash_vla` | `vp.handoff(env, ...)` | **シミュレータの物理状態** |
...
ここが `bdash_scripted` には無い、VLA 評価固有の実装。
...
- しかし `InsertionController` は 7-vector の **相対 IK** `[dx,dy,dz,ax,ay,az,grip]` を出す。
  → `_ik_to_joint`（`bdash_vla_policy.py:171`）で 8-dof 関節へ変換する。
...
> **切り分け用のデバッグ経路**: 環境変数 `BDASH_VLA_SCRIPTED_PICK` を立てると前段を VLA の代わりに
> スクリプト専門家ピックに差し替えられる（IK→関節ブリッジ単体の検証用 — 把持が確実なのに嵌合が
> 失敗するならブリッジ側のバグ）。`BDASH_VLA_FORCE_INSERT` は step 0 から全 env を後段に固定する。
...
`isaaclab_arena_gr00t/policy/config/bdash_pick_insert_gr00t_closedloop_config.yaml`。

| 項目 | 値 |
|---|---|
| モデル | GR00T N1.6（`datasets/bdash/vla_pick_handoff` で微調整） |
...
（`bdash_pick_handoff_v2/v3_config.yaml`）:
...
この変遷の結果、現行の `bdash_vla` 評価は `franka_joint`（8-dof 関節）で動き、`bdash_vla_policy.py`
...
### 6.2 主な閾値（`configs/bdash/peg_insert/controllers.yaml` の `insertion:`）
...
docker exec isaaclab_arena-latest bash -c "cd /workspaces/isaaclab_arena && \
  ...
  --policy_config_yaml_path isaaclab_arena_gr00t/policy/config/bdash_pick_insert_gr00t_closedloop_config.yaml \
  --num_envs 1 --enable_cameras --headless --num_episodes 20 \
  bdash_pick_insert --embodiment franka_joint --clearance 2.0 --level L1"

# グリッド評価（bdash_scripted / bdash_rule_based / bdash_vla を横断）
/isaac-sim/python.sh scripts/bdash/run_eval_grid.py --policies bdash_vla --clearances 2.0 --levels L1
```
...
2. **学習の切り出し点とランタイムの切り替え点が同一述語。**
   `record_vla_demos.py --until handoff` と `BDashVLAPolicy` のラッチは同じ `vp.handoff`
...
| 役割 | パス |
|---|---|
| VLA→ルール スイッチ本体 | `isaaclab_arena/policy/bdash_vla_policy.py` |
| §3.4 述語（`handoff` / `grasped` / `inserted`） | `isaaclab_arena_environments/mdp/bdash_peg_predicates.py` |
...
| VLA 評価設定 | `isaaclab_arena_gr00t/policy/config/bdash_pick_insert_gr00t_closedloop_config.yaml` |
| 述語/幾何 閾値 | `configs/bdash/peg_insert/task.yaml` |
| コントローラ ゲイン | `configs/bdash/peg_insert/controllers.yaml` |
| 教師データ記録（切り出し点） | `scripts/bdash/record_vla_demos.py` |
| データセット設定（v2/v3 = `processed_actions` 8-dof） | `isaaclab_arena_gr00t/lerobot/config/bdash_pick_handoff_v{2,3}_config.yaml` |
| 比較ポリシー（scripted / rule_based） | `isaaclab_arena/policy/bdash_scripted_policy.py` / `bdash_rule_based_policy.py` |
| グリッド評価ランナー | `scripts/bdash/run_eval_grid.py` |
